...
public class TaskService {
    ...
    public String createTask(TaskRequest dto) {
        ...
        // Restricción para evitar crear tareas en fechas anteriores a la actual
        if (
            dto.getDueDate() != null &&
            dto.getDueDate().isBefore(LocalDate.now())
        ) {
            throw new RuntimeException(
                "La fecha de vencimiento no puede ser a la de hoy"
            );
        }
        ...
        if (dto.getIdGeoPoint() != null) {
            task.setGeoPoint(
                geoPointService.getGeoPointById(dto.getIdGeoPoint())
            );
        }
    ...
    public Map<String, Object> getSectorWithMostCompletedTasksWithinRadius(
        Long userId,
        double radiusInKm
    ) {
        double radiusInMeters = radiusInKm * 1000.0;
        return taskRepository.findSectorWithMostCompletedTasksWithinRadius(
            userId,
            radiusInMeters
        );
    }
