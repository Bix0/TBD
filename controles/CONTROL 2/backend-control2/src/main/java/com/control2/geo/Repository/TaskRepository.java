...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(
        value = "SELECT gp_task.sector AS sector, COUNT(t.id_task) AS total_tareas " +
            "FROM task t " +
            "JOIN geo_point gp_task ON t.id_geo_point = gp_task.id_geo_point " +
            "CROSS JOIN users u " +
            "JOIN geo_point gp_user ON u.id_geo_point = gp_user.id_geo_point " +
    ...
    @Query(
        value = "SELECT COALESCE(AVG(ST_Distance(gp_task.point::geography, gp_user.point::geography)), 0) " +
            "FROM user_tasks ut " +
    ...
    @Query(
        value = "SELECT gp.sector AS sector, COUNT(t.id_task) AS total_pendientes " +
            "FROM task t " +
    ...
    @Query(
        value = "SELECT gp.sector AS sector, COUNT(ut.id_task) AS total_tareas " +
            "FROM user_tasks ut " +
            "JOIN task t ON ut.id_task = t.id_task " +
            "JOIN geo_point gp ON t.id_geo_point = gp.id_geo_point " +
    ...
    List<Map<String, Object>> countTasksByUserAndSector(
        @Param("idUser") Long idUser
    );
}
