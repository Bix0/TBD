...
const emit = defineEmits(['save-edit', 'delete-task', 'complete-task', 'view-map'])
...
<template>
  <li class="task-item" :class="{ 'completed-style': task.status === 'Completada' }">
    ...
    <div v-else class="task-view">
      <div class="task-info">
        <div class="title-row">
            <h3>{{ task.title }}</h3>
            <span class="status-badge" :class="task.status === 'Completada' ? 'badge-success' : 'badge-warning'">
                {{ task.status }}
            </span>
        </div>
        <p>{{ task.description }}</p>
        <small>Vence el: {{ formatForDisplay(task.dueDate) }} | Sector: {{ task.geoPoint?.sector || 'N/A' }}</small>
      </div>
...
.badge-warning { background: #ffe082; color: #7f6000; }
.badge-success { background: #b9f6ca; color: #00796b; }

.task-info p { margin: 0 0 6px; color: var(--text); }
.task-info small { color: var(--text); opacity: 0.7; }
...
.map-btn, .edit-btn, .complete-btn, .delete-btn {
  padding: 6px 12px;
  border: none;
  border-radius: 6px;
  cursor: pointer;
...
.edit-btn { background: #ffc107; color: #121212; }
.complete-btn { background: #00e676; color: #121212; }
.delete-btn { background: #ff5252; color: white; }

.map-btn:hover, .edit-btn:hover, .complete-btn:hover, .delete-btn:hover { opacity: 0.9; }

.edit-form-inline { display: flex; flex-direction: column; gap: 10px; width: 100%; }
.edit-form-inline input, .edit-form-inline select { padding: 8px; border-radius: 6px; border: 1px solid var(--border); background: var(--code-bg); color: var(--text); }
.edit-actions { display: flex; gap: 8px; justify-content: flex-end; }
.save-btn { padding: 8px 16px; background: var(--accent); color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; }
.cancel-btn { padding: 8px 16px; background: transparent; border: 1px solid var(--border); color: var(--text-h); border-radius: 6px; cursor: pointer; }
</style>