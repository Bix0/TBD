...
function Personajes() {
  ...
  const [activoId, setActivoId] = useState(
    localStorage.getItem("activePersonajeId"),
  );
  ...
  const crearPersonaje = (e) => {
    ...
    // CORRECCIÓN: El backend espera IDs planos (camelCase), no objetos anidados
    const nuevoPersonaje = {
      jugadorId: userId, // String con el ObjectId del jugador logueado
    ...
    api
      .post("/api/personajes", nuevoPersonaje)
      .then((res) => {
  ...
  const subirNivel = (personaje) => {
    ...
    api
      .put(`/api/personajes/${personaje.idPersonaje}`, personajeActualizado)
  ...
  const bajarNivel = (personaje) => {
    if (personaje.nivel <= 1)
      return alert("Un personaje no puede tener un nivel menor a 1.");
    const personajeActualizado = {
      ...personaje,
      nivel: personaje.nivel - 1,
      itemLevel: personaje.itemLevel - 1, // camelCase
  ...
  const borrarPersonaje = (id) => {
    if (
      window.confirm(
        "¿Estás seguro? Se borrará su inventario y progreso para siempre.",
      )
    ) {
      api
  ...
  const salirDelClan = (personaje) => {
    if (!personaje.clan)
      return alert("El personaje no pertenece a ningún clan.");
    const idClan = personaje.clan.idClan || personaje.clan.id_clan || 1;
    const pjId = personaje.idPersonaje || personaje.id_personaje;
    if (
      window.confirm(
        `¿Seguro que deseas que ${personaje.nombre} salga del clan ${personaje.clan.nombre || ""}?`,
      )
    ) {
      api
          ...
          >
            ...
            {personajes.map((p) => (
              ...
              >
                ...
                <h2 style={{ color: "#81c784", margin: "0 0 5px 0" }}>
                  {p.nombre}
                </h2>
                ...
                >
                  🏰 Clan: <b>{p.clan ? p.clan.nombre : "Sin Clan"}</b>
                </p>
                <p style={{ color: "#61dafb", margin: "0 0 5px 0" }}>
                  Nivel: {p.nivel} | Poder de Combate:{" "}
                  <b>{p.itemLevel ?? p.item_level ?? 0} iLvl</b>
                </p>
                <p
                  style={{
                    color: "#ffb74d",
                ...
                >
                  ⚔️ Rol: <b>{p.rolClan || p.rol_clan || p.rol || "Sin Rol"}</b>
                </p>
                ...
                >
                  <button
                    onClick={() => seleccionarActivo(p.idPersonaje)}
                    style={{
                      flex: "1 1 45%",
                      padding: "8px",
                  ...
                  <button
                    onClick={() => borrarPersonaje(p.idPersonaje)}
                    style={{
                      flex: "1 1 45%",
                      padding: "8px",
